...
unsigned int irq_number; /* Khai báo irq number */

#define DRIVER_NAME "gpiodevice"
#define DRIVER_CLASS "gpioclass"

/*
 * @Hàm này được gọi mổi khi có ngắt từ GPIO
 */
static irq_handler_t my_handler(unsigned int irq, void *dev_id, struct pt_regs *regs) {
	printk(KERN_DEBUG"%s: Interrupt received from GPIO2\n",__func__);
...
static int __init ModuleInit(void) {
	...
	/* Đăng ký interrupt */
	if(request_irq(irq_number, my_handler, IRQF_TRIGGER_RISING, "gpio2_irq", NULL) != 0){
	printk(KERN_DEBUG"Failed to request interrupt number : %d\n", irq_number);
		goto gpio2_failed;
	}
	printk(KERN_DEBUG"GPIO 2 is mapped to IRQ number: %d\n", irq_number);
...
MODULE_DESCRIPTION("GPIO driver-Interrupt example");
